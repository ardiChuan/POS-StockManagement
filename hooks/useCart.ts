...
function cartReducer(state: CartState, action: CartAction): CartState {
  switch (action.type) {
    case "ADD_ITEM": {
      ...
      if (existing) {
        // Fish are always qty=1 and unique — don't merge
...
export function useCart() {
  ...
  const setCustomer = useCallback((id: string | null, name: string, phone: string) =>
    dispatch({ type: "SET_CUSTOMER", id, name, phone }), []);
  const setDiscount = useCallback((discount_type: DiscountType | null, discount_value: number) =>
